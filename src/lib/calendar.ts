...
// Legacy single-select (kept so other code keeps compiling)
...
/** Reminder offsets in minutes for an item (merging legacy + new). */
...
export function expandOccurrences(
  ...
  for (const it of items) {
    ...
    const push = (s: Date) => {
      ...
      if (e < rangeStart || s > rangeEnd) return false;
      out.push({
        ...it,
        instanceStart: s,
        instanceEnd: e,
    ...
    if (rule.freq === "weekly" && rule.byWeekDays && rule.byWeekDays.length > 0) {
      // Walk week by week from start; emit every selected weekday
...
/** Human-readable summary of a recurrence rule (Vietnamese). */

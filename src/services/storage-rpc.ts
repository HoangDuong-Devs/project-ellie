...
export const getStorageValueServer = createServerFn({ method: "POST" }).handler(
  async ({ data }) => {
...
export const setStorageValueServer = createServerFn({ method: "POST" }).handler(
  async ({ data }) => {
    if (!isRecord(data) || typeof data.key !== "string") {
...
export const deleteStorageValueServer = createServerFn({ method: "POST" }).handler(
  async ({ data }) => {
    if (!isRecord(data) || typeof data.key !== "string") {
